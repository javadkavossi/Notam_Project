...
// TODO!
// for SOLCLIENT_SESSION_PROP_SSL_EXCLUDED_PROTOCOLS
...
struct options
{
    ...
    char            *tlsMinProtocol_p;
    char            *tlsMaxProtocol_p;
...
#define USAGE_PARAMS      \
        ...
        "\t-C  --cipher=ciphers List of comma separated cipher suites (TLSv1.2 and under only).\n"\
        "\t-m  --minProt=Minimum TLS protocol version\n"\
        "\t-M  --minProt=Maximum TLS protocol version\n"\
...
{
    static char    *optstring = "c:l:n:p:u:m:M:C:N:T:ijE:Y:W:k:d:";
    ...
    opt->tlsMinProtocol_p = NULL;
    opt->tlsMaxProtocol_p = NULL;
    ...
    while ( ( c = getopt_long ( argc, argVOS, optstring, longopts, NULL ) ) != -1 ) {
        switch ( c ) {
            ...
            case 'm':
                strLen = strlen(optarg);
                opt->tlsMinProtocol_p = (char *)malloc(strLen+1);
                if (opt->tlsMinProtocol_p == NULL) {
                    solClient_log ( SOLCLIENT_LOG_ERROR, "malloc failed: out of memory!"); 
                    exit (-1);
                }
                strncpy ( opt->tlsMinProtocol_p, optarg, strLen +1 );
                break;
            case 'M':
                strLen = strlen(optarg);
                opt->tlsMaxProtocol_p = (char *)malloc(strLen+1);
                if (opt->tlsMaxProtocol_p == NULL) {
                    solClient_log ( SOLCLIENT_LOG_ERROR, "malloc failed: out of memory!"); 
                    exit (-1);
                }
                strncpy ( opt->tlsMaxProtocol_p, optarg, strLen +1 );
                break;
            case 'i':
...
main ( int argc, char *argv[] )
{
    ...
    if (commandOpts.tlsMinProtocol_p != NULL) {
      sessionProps[propIndex++] = SOLCLIENT_SESSION_PROP_MINIMUM_TLS_PROTOCOL_VERSION;
      sessionProps[propIndex++] = commandOpts.tlsMinProtocol_p;
    }
    if (commandOpts.tlsMaxProtocol_p != NULL) {
      sessionProps[propIndex++] = SOLCLIENT_SESSION_PROP_MAXIMUM_TLS_PROTOCOL_VERSION;
      sessionProps[propIndex++] = commandOpts.tlsMaxProtocol_p;
    }
  ...
  notInitialized:
    ...
    if (commandOpts.tlsMinProtocol_p != NULL) {
      free((void *)commandOpts.tlsMinProtocol_p);
    }   
    if (commandOpts.tlsMaxProtocol_p != NULL) {
      free((void *)commandOpts.tlsMaxProtocol_p);
    }   
