...
## بازخورد بازبینی کارشناس دیسپچ (ورودی برای نسخهٔ ۱.۱)

سند [EXPERT_REVIEW.md](EXPERT_REVIEW.md) توسط کارشناس تکمیل شد. جمع‌بندی:

### اعمال‌شده در `1.1.0` (بی‌ابهام و کاهش‌دهندهٔ نویز)

| تغییر | از | به | دلیل کارشناس |
|-------|:--:|:--:|--------------|
| وضعیت `CN` (لغو) | −۵۰ | **−۴۰** | «−۵۰ خیلی منفی بود، −۴۰ خوبه» |
| وضعیت `TT` (Trigger) | ۰ | **−۴۵** | Trigger NOTAM اطلاعی است؛ نمونهٔ QFALT/QFATT نباید بحرانی شود |

اثر روی داده واقعی: Trigger از HIGH(78)→LOW(33)؛ توزیع INFO/LOW متعادل‌تر شد.

### تأییدشده (بدون تغییر)

بیشتر تعدیل‌های وضعیت و پرچم‌های AD_CLOSED/RWY_CLOSED/FICON/ILS «OK». «فرودگاه بسته = بحرانی» و «باند بسته = بحرانی» تأیید شد.

### وزن‌های پایهٔ پیشنهادی کارشناس (بخش ۱) — **معلق تا لایهٔ Operational Impact**

کارشناس اعداد جدیدی داد (مثلاً MET ۴۵→۷۰، COMMS ۴۰→۶۰، NAVIGATION ۴۵→۶۰، ATS ۵۵→۶۷، AIRSPACE ۵۵→۶۰، TAXIWAY ۳۸→۳۵، OBSTACLE ۴۵→۴۰).

> ⚠️ **چرا هنوز اعمال نشده:** خودِ کارشناس در بخش ۵ نشان داد توزیع درست باید HIGH ~٪۶ باشد (نه ٪۴۹). افزایشِ صرفِ وزن‌های پایه، بدون لایهٔ Operational Impact که در بیشتر موارد امتیاز را **پایین** می‌آورد (اثر بی‌ربط به این پرواز/هواپیما/مسیر)، گرگ‌گویی را **بدتر** می‌کند. این اعداد به‌عنوان «سقفِ اثر هر دسته» ثبت شده و **همراه با** لایهٔ Impact در نسخهٔ بعد اعمال می‌شوند.

### نمونه‌های اختلافی که کارشناس اصلاح کرد (ورودی لایهٔ Impact)

| NOTAM | سطح سیستم | نظر کارشناس | فاکتور بستر |
|-------|:--:|:--:|------|
| باند بسته | بحرانی ۱۰۰ | **«اگر تک‌باند ۱۰۰، وگرنه ۷۰»** | تعداد باند فرودگاه |
| QFALT محدودیت stand | بحرانی ۸۶ | **متوسط ۵۵** | محدودیت پذیرش، نه بستن |
| سنسور باد U/S | بالا ۶۷ | **متوسط ۵۵** | — |
| سوخت 100LL نیست | متوسط ۵۰ | **پایین ۲۰** | «سوخت این هواپیماها نیست» → بی‌ربط |
| RFFS ناکافی | بحرانی ۸۴ | **شرطِ پیش‌پرواز**، نه حین‌پرواز | نوع/دستهٔ هواپیما |

این‌ها دقیقاً «امتیاز نهاییِ وابسته به پرواز»اند و در [لایهٔ Operational Impact](ANALYSIS_AND_BRIEFING.md#۱۰-لایهٔ-operational-impact-e55--بازخورد-کارشناس-دیسپچ) پیاده می‌شوند.

### پرچم‌های پیشنهادی کارشناس (فهرست کامل — برای لایهٔ Impact)

کارشناس ~۲۴ پرچم با bonus پیشنهاد داد که بیشترشان متن‌محور/بستر‌محورند و در لایهٔ Impact پیاده می‌شوند. نمونه‌های مهم:
`Snow/Ice/Slush/Water +10`، `RVR Unavailable +7`، `GNSS Outage +7`، `Runway Weight (PCN) +6`، `Fuel Not Available +9`، `LVP Active +9`، `Approach Procedure Suspended +7`، `Drone/UAS +4`، `Main Taxiway Closed +5`، `PAPI/Approach Lighting Failure +5`. فهرست کامل در EXPERT_REVIEW (بازگشتی کارشناس).
